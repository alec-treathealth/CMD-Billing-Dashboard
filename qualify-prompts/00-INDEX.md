...
- **Test floors** are defined only by CLAUDE.md's *Verification gate*. Read that gate before running any wave; re-measure on a clean detached `origin/main` checkout rather than relying on a copied count.
