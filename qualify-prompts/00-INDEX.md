...
- **Test floors** are NOT stated in these prompts any more — read them from CLAUDE.md's *Verification gate*, the only place they are re-measured. ⚠ This line used to say the waves' `≥1439` / `≥831` "match CLAUDE.md's ratified counts", and by 2026-08-30 that claim was **false and dangerous**: CLAUDE.md had moved to 1886 / 1038, so the waves were 447 root and 207 app tests low and a session trusting this sentence would have checked a suite that had lost 447 tests against a number it still passed. A prompt asserting agreement with CLAUDE.md is a LIVE copy, not a historical record — which is exactly why the numbers are gone rather than updated.
