...
/** Default wall-clock budget before the loop stops launching NEW customers (ms). Leaves ample
 *  headroom under the 300s Vercel function for the post-loop cache revalidate — the ~58s
 *  charge-rollup refresh now runs in its OWN dedicated cron (/api/cron/refresh-charge-rollup),
 *  not here. Unfinished customers catch up next run (idempotent + self-healing), so a tighter
 *  budget only lengthens the tail on the heaviest days — it never drops data. Value unchanged
 *  (210s) — retuning now that the refresh moved out is a separate decision. */
