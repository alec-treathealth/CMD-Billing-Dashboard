# ARCHIVED — CLAUDE.md as of 2026-07-06

> **FROZEN. This is NOT the project's context file and is NOT loaded by Claude
> Code.** It was last updated at commit `0f688d2` (2026-07-06) and is **234
> commits stale** as of 2026-07-28. It is retained for the reasoning and
> phase history it records, not for facts.
>
> **The live context is `/CLAUDE.md` at the repo root, plus the path-scoped
> rules in `.claude/rules/`.** Where this file conflicts with those — or with
> `docs/veris-data-notes.md` — it loses.
>
> Known-wrong here: "the CMD cron is the only scheduled writer" (there are 10
> crons) · "239 tests" (697 root / 127 app) · `/claims` is live (it is a
> `redirect('/')` stub since 2026-07-15) · 0027/0028 "shelved and uncommitted"
> (committed; the collections plane is multi-tenant via 0030–0033) · migrations
> "0001–0011"/"0006–0022" (through 0071) · three dashboard tabs (two, plus
> top-level `/qualify` and `/billing-audit`) · next Veris migration 020 (021).
> The BXR-only collections tenancy guardrail in §7/§15 is **satisfied and
> closed** — do not re-apply it.
