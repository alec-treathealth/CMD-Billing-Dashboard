...
const W60 = trailingWindow(60);
const W90 = trailingWindow(90);
const M_JUL = { kind: 'month' as const, year: 2026, month: 7 };
...
const RESOLVED: QualifyCohort = { payer: 'AETNA', facility: 'fac-a', window: W60 };

test('RESOLVE_PAYER — sets payer/facility/window (a brand-new cohort)', () => {
  const next = cohortReducer(RESOLVED, { type: 'RESOLVE_PAYER', payer: 'CIGNA', facility: 'fac-x', window: W30 });
  assert.deepEqual(next, { payer: 'CIGNA', facility: 'fac-x', window: W30 });
});

test('RESOLVE_PAYER — an unresolved search (null payer/facility) is a coherent cohort', () => {
  const next = cohortReducer(RESOLVED, { type: 'RESOLVE_PAYER', payer: null, facility: null, window: W90 });
  assert.deepEqual(next, { payer: null, facility: null, window: W90 });
});
...
test('SWITCH_FACILITY — sets facility, PRESERVES payer+window', () => {
  ...
  assert.deepEqual(next, { payer: 'AETNA', facility: 'fac-b', window: W60 });
});

test('CHANGE_WINDOW — sets window, PRESERVES facility (no rank-1 teleport) + payer', () => {
  const next = cohortReducer(RESOLVED, { type: 'CHANGE_WINDOW', window: M_JUL });
  assert.deepEqual(next, { payer: 'AETNA', facility: 'fac-a', window: M_JUL }, 'the facility PERSISTS across a window change (calendar too) — the fix');
});
...
test('purity — the reducer never mutates the input state', () => {
  ...
  cohortReducer(start, { type: 'CHANGE_WINDOW', window: M_JUL });
  cohortReducer(start, { type: 'RESOLVE_PAYER', payer: 'X', facility: 'y', window: W90 });
...
test('cohortKey — identity is EXACTLY (payer, facility, window)', () => {
  const base: QualifyCohort = { payer: 'AETNA', facility: 'fac-a', window: W60 };
  assert.notEqual(cohortKey(base), cohortKey({ ...base, facility: 'fac-b' }), 'facility changes the key');
  assert.notEqual(cohortKey(base), cohortKey({ ...base, window: W30 }), 'window changes the key');
...
test('cohortKey — no collision when a label contains the naive delimiter (space/pipe)', () => {
  ...
  const left: QualifyCohort = { payer: 'A B', facility: 'C', window: W60 };
  const right: QualifyCohort = { payer: 'A', facility: 'B C', window: W60 };
