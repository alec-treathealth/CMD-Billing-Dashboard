...
const RESOLVED: QualifyCohort = {
  payer: 'AETNA', facility: 'fac-a', window: 60, prefix: 'ABC', page: 2, cursors: [null, C1, C2],
};
...
test('cohortKey — distinguishes identity fields but IGNORES page/cursors (genRef guards page races)', () => {
  const base: QualifyCohort = { payer: 'AETNA', facility: 'fac-a', window: 60, prefix: 'ABC', page: 0, cursors: [null] };
...
test('cohortKey — no collision when a label contains the naive delimiter (space/pipe)', () => {
  ...
  const left: QualifyCohort = { payer: 'A B', facility: 'C', window: 60, prefix: '', page: 0, cursors: [null] };
  const right: QualifyCohort = { payer: 'A', facility: 'B C', window: 60, prefix: '', page: 0, cursors: [null] };
