...
| Plane | Directory | Next number (as of 2026-09-10) |
|---|---|---|
| Product (`claims`, `collections`) | `supabase/migrations/00NN_*.sql` | **0112** — ⚠ **0109 / 0110 / 0111 (AR Management) ARE ALL APPLIED LIVE 2026-09-09/10** (ledger `20260909103129` / `20260909104221` / `20260910041317`); none of those three numbers is reusable. This cell said **0109** until 2026-09-10, i.e. it advertised a number that was already consumed — exactly the 0096 collision this file exists to prevent, and it auto-loads for the next person who touches `supabase/migrations/`. **Re-derive from the live ledger, never from this cell or a directory listing.** 0110 exists because 0109 shipped claim-only notes and B_PATNOTES TYPE 0 rows carry `CLAIM=0` (1,535 of CAMH's 1,567); 0111 replaced a `select *` in `ar_set_work` and added the staff-role check on the assignee. Earlier: **0108 (`(business_entity_id, charge_date)` btree on the charge rollup) APPLIED LIVE 2026-09-08** as autocommit `execute_sql` CIC + hand-inserted ledger row `20260908223624`; **0107 applied live 2026-08-24** (same path); **0105/0106 applied live 2026-08-18** (0106 in two ledger rows — see CLAUDE.md). Earlier: **0103 (`grant select (business_entity_id, employer_name)` on `collections.cmd_explorer_rows` to `cmd_rollup_writer`) APPLIED LIVE 2026-08-17** via `apply_migration`. It is the THIRD fix in one chain and each failed differently: 0101's UPDATE grant was INERT (RLS on, no UPDATE policy → zero rows, no error); 0102 added the policy; 0103 grants the SELECT that the UPDATE's own WHERE clause requires — Postgres needs SELECT on every column READ in an UPDATE, and the backfill reads `business_entity_id` + `employer_name`, so `--commit` raised 42501 until this landed. Column-scoped on purpose: table-level SELECT is still **false**, and the writer's readable columns are exactly `business_entity_id, employer_name, row_fingerprint`. **0101 + 0102 BOTH APPLIED LIVE 2026-08-15** (`20260815103136` / `20260815103354`); neither number is reusable. ⚠ 0101 was applied as **autocommit `execute_sql`** (two `CREATE INDEX CONCURRENTLY`), which writes **NO ledger row** — its row was inserted by hand. **If you apply outside `apply_migration`, insert the ledger row yourself**, or the next session derives your number as free. 0102 added the UPDATE RLS policy that 0101's grant needed to be anything but inert. Earlier: 0098 applied live 2026-08-11 (`20260811040852`); **0099 (etl_run + pipeline_state) applied live 2026-08-12** (`20260812203336`); **0100 (`facility_assignments_guard` search_path pin) AUTHORED-NOT-APPLIED on `fix/qualify-audit-wave1`**; **0095 consumed its slot with NO file** (one-shot prune, ledger 20260809073608 — never reuse it); 0096 (`manual_deposits`, now tracked) + 0097 (qualify watchers) applied live 2026-08-10 |
| Veris ML (`staging`, `ref`, `core`, `intel`) | `SQL Schemas/0NN_*.sql` | **039** — **038 (`ref.code_description`) applied live 2026-09-08** (`20260908093315`); **037 authored-not-applied on `feat/era-835-claim-line-grain` — consumed, do not reuse**; 035/036 applied live 2026-09-06/07; 029 applied live; 030/031 authored-not-applied (031 held on purpose — see CLAUDE.md); **032/033/034 applied live 2026-08-10** (`intel_writer_select_grant`, `expected_payment_manual_lifecycle`, `drop_expected_payment_manual_live_idx`) |
