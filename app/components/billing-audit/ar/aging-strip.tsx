...
export function AgingStrip({ bands, kpi, selected, onToggle, agedOnly, onToggleAgedOnly, loading }: AgingStripProps) {
  const byBand = new Map(bands.map((b) => [b.band, b]));
  const total = Math.max(1, Number(kpi.balance) || 0);
    ...
    <section aria-label="Open AR by age" className={`transition-opacity duration-150 ${loading ? 'opacity-60' : ''}`}>
      <div className="flex flex-wrap items-end justify-between gap-x-10 gap-y-4">
        <div className="animate-ths-reveal">
          <div className="text-xs font-semibold uppercase tracking-wide text-ink400">Open AR · {kpi.claims.toLocaleString('en-US')} {claimsWord}</div>
          <div className="font-display mt-1 text-[2.75rem] font-medium leading-none tracking-tight text-[var(--brand-ink)] sm:text-[3.25rem]">
            {moneyWhole(kpi.balance)}
          </div>
        ...
        <div className="flex flex-wrap gap-x-8 gap-y-3 animate-ths-reveal" style={{ animationDelay: `${staggerDelayMs(1)}ms` }}>
          <Kpi label="Denied" value={kpi.denied.toLocaleString('en-US')} sub={moneyCompact(kpi.denied_balance)} tone={kpi.denied > 0 ? 'danger' : undefined} />
          <Kpi label="Being worked" value={kpi.worked.toLocaleString('en-US')} sub={kpi.claims > 0 ? `${Math.round((kpi.worked / kpi.claims) * 100)}% of open` : undefined} />
          <Kpi label="Follow-up overdue" value={kpi.followup_overdue.toLocaleString('en-US')} sub="CMD follow-up date passed" tone={kpi.followup_overdue > 0 ? 'warn' : undefined} />
          <Kpi label="Never noted" value={kpi.never_noted.toLocaleString('en-US')} sub="no CMD or app note" />
        </div>
      </div>
          ...
          return (
            ...
            >
              ...
              <span className="mt-2 block">
                <span className="font-display block text-xl font-medium leading-none text-ink900">{moneyCompact(balance)}</span>
                <span className="ths-num mt-1 block text-xs text-ink400">{claims.toLocaleString('en-US')} {claims === 1 ? 'claim' : 'claims'} · {Math.round(share * 100)}%</span>
              </span>
              <span aria-hidden className="mt-3 block h-1 w-full overflow-hidden rounded-full bg-ground">
                <span className="block h-full rounded-full transition-[width] duration-300 ease-out" style={{ width: `${width}%`, backgroundColor: color }} />
              </span>
            </button>
          );
        })}
